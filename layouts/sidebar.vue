  ...
  <div class="wrapper">
    <nav id="sidebar">
          ...
          <li>
            ...
            >
              <b-row class="adminStyle">
                <b-col cols="2">
                  <span><font-awesome-icon icon="user-tie" class="icon"/></span>
                </b-col>
                <b-col>
      ...
      <footer class="footerStyle">
        Copyright &copy; 2024 Koronadal City
      </footer>
    </nav>
  ...
  methods: {
    ...
    adminwasclicked() {
      ...
      if(this.clickStatus == true){
        document.getElementById("adminCaretToggle").style.transform = "rotate(-180deg)";
        document.getElementById("adminCaretToggle").style.transition = "all .3s linear";
        // document.getElementById("adminCaretToggle").setAttribute( "style", "transition: all .3s linear; transform: rotate(180);");
      }else{
        document.getElementById("adminCaretToggle").style.transform = "rotate(0deg)";
        document.getElementById("adminCaretToggle").style.transition = "all .3s linear";
        // document.getElementById("adminCaretToggle").setAttribute( "style", "transition: all .3s linear; transform: rotate(180);");
      }
      ...
      try {
        ...
        this.socket.on(`new req - ${id}`, data => {
         ...
         // start words of the notif
          ...
          const time = new Date('1970-01-01T' + hour[1] + 'Z').toLocaleTimeString('en-US',
            {timeZone:'UTC',hour12:true,hour:'numeric',minute:'numeric'}
          )

          this.notifs.unshift({ id: e.id, msg, is_read: e.is_read, date_updated: time, request_id: e.request_id, data: e, currentUser: localStorage.id, user: user, pre: pre });
        ...
        this.socket.on(`my req - ${id}`, data => {
          ...
          for (let i = 0; i < notifs.length; i++) {
            ...
            temp.push({ id: e.id, msg, is_read: e.is_read, date_updated: time, request_id: e.request_id, data: e, currentUser: localStorage.id, user: user, pre: pre });
          }
